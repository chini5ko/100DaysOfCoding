...
bot.on('message', async function(msg) {
  ...
  if (receiver.id === 'filehelper') {
    ...
    else if (text === '#airplane') { //lines added by Chini5ko
      busyIndicator = true
      await filehelper.say('Airplane Mode ON')
      busyOn = "airplane";
    }
   ...
   */
  if (!room) {
    console.log(busyOn + "Mode")

    if(busyOn=='swim'){
      busyAnnouncement = 'I am Nisko Bot (Hector Digital assistant) \n When he gets back from the Pool, he might be tired, but he will get back to you when ASAP'
    }

    if(busyOn=='sleep'){
      busyAnnouncement = 'I am Nisko Bot (Hector Digital assistant) \n Hector is sleeping now. He will wake up at 6:00 Am eastern time. He might get in touch with you at 7:30 AM.'
    }

    if(busyOn=='airplane'){
      busyAnnouncement = 'I am Nisko Bot (Hector Digital assistant) \n Hector is currently on a Airplane, and he will get back to you ASAP once he touch land. '
    }
    console.log(busyAnnouncement)
